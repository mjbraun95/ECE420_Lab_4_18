...
int main(int argc, char* argv[]){
    ...
    // Determine local work range, considering uneven division of nodes
    int remainder = nodecount % world_size;
    int offset = 0;
    local_start = world_rank * (nodecount / world_size) + (world_rank < remainder ? world_rank : remainder);
    local_end = (world_rank + 1) * (nodecount / world_size) + ((world_rank + 1) <= remainder ? (world_rank + 1) : remainder);
    ...
    do {
        ...
        // Start asynchronous gather operation
        MPI_Iallgather(r + local_start, local_end - local_start, MPI_DOUBLE,
                      r_global, local_end - local_start, MPI_DOUBLE, MPI_COMM_WORLD, &gather_request);
        
        // Wait for the asynchronous gather operation to complete
